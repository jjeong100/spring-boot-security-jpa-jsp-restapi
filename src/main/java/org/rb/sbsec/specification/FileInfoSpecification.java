...
public class FileInfoSpecification implements Specification<FileInfo> {

    private FileInfo filter;

    public FileInfoSpecification(FileInfo filter) {
        super();
        this.filter = filter;
    }

    @Override
    public Predicate toPredicate(Root<FileInfo> root, CriteriaQuery<?> cq,
            CriteriaBuilder cb) {

        Predicate p = cb.disjunction();

        if (filter.getFileName() != null) {
            p.getExpressions().add(cb.like(root.get("fileName"), "%" + filter.getFileName() + "%"));
        }
//
